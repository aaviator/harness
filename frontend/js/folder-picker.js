...
export class FolderPicker {
  ...
  async _expand(path, parentNode) {
    ...
    try {
      ...
      // Add ".." parent link (unless at root)
      if (path !== "/") {
        const parent = path.replace(/\/[^/]+\/?$/, "") || "/";
        const dotdot = document.createElement("div");
        dotdot.className = "flex items-center gap-1 py-0.5 px-2 rounded cursor-pointer hover:bg-slate-700 text-slate-400 text-sm font-mono";
        dotdot.textContent = "..";
        dotdot.title = parent;
        dotdot.onclick = () => this.init(parent);
        childContainer.appendChild(dotdot);
      }

      const dirs = (data.entries || []).filter(e => e.isDir);
      if (dirs.length === 0 && path === "/") {
        const empty = document.createElement("div");
        empty.className = "text-slate-600 text-xs px-2 py-1";
      ...
      if (childContainer) {
        ...
        err.className = "text-red-500 text-xs px-2";
        err.textContent = "Error loading";
        childContainer.appendChild(err);
      }
    }
  }
}
