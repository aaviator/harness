...
export class ClaudeStream {
  ...
  async send({ message, workingDirectory, sessionId, allowedTools, permissionMode,
               useSudo, model, maxThinkingTokens, customSystemPrompt, appendSystemPrompt, maxTurns, tmuxSession }) {
    this.aborted = false;
  ...
  _processLine(line) {
    ...
    switch (envelope.type) {
      ...
      case "ask_user":
        ...
        dispatch("stream:ask_user", { toolUseId: envelope.toolUseId, question: envelope.question, suggestions: envelope.suggestions ?? [] });
        return;
