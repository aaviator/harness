...
export async function handleGenerateTitle(c: Context): Promise<Response> {
  const { firstMessage, firstResponse } = await c.req.json<{
    firstMessage: string;
  ...
  const prompt = `Write a short title (5 words max) for a conversation that starts with:
User: ${firstMessage.slice(0, 300)}
