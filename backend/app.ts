...
import { handleChatRequest, handleToolResult, isConvActive, abortConv } from "./handlers/chat.ts";
import { handleAbortRequest } from "./handlers/abort.ts";
import { handleFsList, handleFsMkdir, handleFsRead, handleFsWrite, handleFsRename, handleFsDelete, handleFsDownload, handleFsUpload } from "./handlers/fs.ts";
...
import { handleCompactRequest } from "./handlers/compact.ts";
...
export function createApp(runtime: Runtime, config: CreateAppConfig): Hono<ConfigContext> {
  ...
  app.get("/api/fs/read", (c) => handleFsRead(c));
  app.post("/api/fs/write", (c) => handleFsWrite(c));
  app.post("/api/fs/rename", (c) => handleFsRename(c));
  app.post("/api/fs/delete", (c) => handleFsDelete(c));
  app.get("/api/fs/download", (c) => handleFsDownload(c));
  app.post("/api/fs/upload", (c) => handleFsUpload(c));
  ...
  app.get("/api/convs/:id/active", (c) => c.json({ active: isConvActive(c.req.param("id")) }));
  ...
  // Compact
  app.post("/api/compact", (c) => handleCompactRequest(c));
  ...
  // Chat
  app.post("/api/chat", bodyLimit({ maxSize: 1 * 1024 * 1024 }), (c) => handleChatRequest(c, requestAbortControllers));
  ...
  app.post("/api/convs/:id/abort", (c) => {
    const ok = abortConv(c.req.param("id"), requestAbortControllers);
    return c.json({ ok });
  });
